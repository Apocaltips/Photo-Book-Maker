...
export async function POST(
  ...
  try {
    ...
    const project = await updateProject(
      projectId,
      (current) => finalizeProject(current),
      {
        expectedRevision: body.expectedRevision,
        activity: {
          actorEmail: auth.user.email,
          actorId: auth.user.id,
          message: `${auth.user.name} finalized the proof for print review.`,
    ...
    return mutationErrorResponse(error, "Unable to finalize this project.");
  }
}
